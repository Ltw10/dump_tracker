...
--
-- All app objects live in PostgreSQL schema dump_tracker (not public).
-- Dashboard → Project Settings → API → Exposed schemas: add dump_tracker.
-- The web client sets db.schema to dump_tracker (see src/supabase.js).
...
CREATE TABLE IF NOT EXISTS dump_tracker.dt_users (
...
COMMENT ON COLUMN dump_tracker.dt_users.leaderboard_opt_in IS 'Whether the user has opted in to appear on the leaderboard';
COMMENT ON COLUMN dump_tracker.dt_users.location_tracking_opt_in IS 'Whether the user has opted in to location tracking features';

-- dt_locations: one row per user per location (name + count + optional address)
CREATE TABLE IF NOT EXISTS dump_tracker.dt_locations (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES dump_tracker.dt_users(id) ON DELETE CASCADE,
...
COMMENT ON COLUMN dump_tracker.dt_locations.address IS 'Street address or location description provided by user';
COMMENT ON COLUMN dump_tracker.dt_locations.latitude IS 'Latitude coordinate if GPS location was provided';
COMMENT ON COLUMN dump_tracker.dt_locations.longitude IS 'Longitude coordinate if GPS location was provided';
COMMENT ON COLUMN dump_tracker.dt_locations.location_data_provided IS 'Whether specific location data (address or GPS) has been provided';
COMMENT ON COLUMN dump_tracker.dt_locations.location_data_declined IS 'Whether user has declined to provide location data for this location';

CREATE INDEX IF NOT EXISTS idx_dt_locations_user_id ON dump_tracker.dt_locations(user_id);
CREATE INDEX IF NOT EXISTS idx_dt_locations_location_name ON dump_tracker.dt_locations(location_name);

-- dt_entries: individual dump events at a location (supports decrement / type)
CREATE TABLE IF NOT EXISTS dump_tracker.dt_entries (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  location_id UUID NOT NULL REFERENCES dump_tracker.dt_locations(id) ON DELETE CASCADE,
  user_id UUID NOT NULL REFERENCES dump_tracker.dt_users(id) ON DELETE CASCADE,
...
ALTER TABLE dump_tracker.dt_entries ADD COLUMN IF NOT EXISTS explosive_dump BOOLEAN DEFAULT FALSE;
ALTER TABLE dump_tracker.dt_entries ADD COLUMN IF NOT EXISTS wipes INTEGER;
CREATE INDEX IF NOT EXISTS idx_dt_entries_explosive_dump ON dump_tracker.dt_entries(explosive_dump) WHERE explosive_dump = TRUE;

CREATE INDEX IF NOT EXISTS idx_dt_entries_location_id ON dump_tracker.dt_entries(location_id);
CREATE INDEX IF NOT EXISTS idx_dt_entries_user_id ON dump_tracker.dt_entries(user_id);
CREATE INDEX IF NOT EXISTS idx_dt_entries_created_at ON dump_tracker.dt_entries(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_dt_entries_ghost_wipe ON dump_tracker.dt_entries(ghost_wipe) WHERE ghost_wipe = TRUE;
CREATE INDEX IF NOT EXISTS idx_dt_entries_messy_dump ON dump_tracker.dt_entries(messy_dump) WHERE messy_dump = TRUE;
CREATE INDEX IF NOT EXISTS idx_dt_entries_classic_dump ON dump_tracker.dt_entries(classic_dump) WHERE classic_dump = TRUE;
CREATE INDEX IF NOT EXISTS idx_dt_entries_liquid_dump ON dump_tracker.dt_entries(liquid_dump) WHERE liquid_dump = TRUE;

-- dt_notifications: feed of events for opted-in users
CREATE TABLE IF NOT EXISTS dump_tracker.dt_notifications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  type TEXT NOT NULL,
  actor_user_id UUID NOT NULL REFERENCES dump_tracker.dt_users(id) ON DELETE CASCADE,
  payload JSONB,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_dt_notifications_created_at ON dump_tracker.dt_notifications(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_dt_notifications_type ON dump_tracker.dt_notifications(type);

-- Dump Tracker: update updated_at on dt_locations
CREATE OR REPLACE FUNCTION dump_tracker.dt_update_updated_at_column()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SET search_path = dump_tracker, pg_temp;

DROP TRIGGER IF EXISTS dt_update_locations_updated_at ON dump_tracker.dt_locations;
CREATE TRIGGER dt_update_locations_updated_at
  BEFORE UPDATE ON dump_tracker.dt_locations
  FOR EACH ROW
  EXECUTE FUNCTION dump_tracker.dt_update_updated_at_column();

-- Create dt_users row when a new auth user is created
CREATE OR REPLACE FUNCTION dump_tracker.dt_handle_new_user()
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO dump_tracker.dt_users (id, email, first_name, last_name)
  VALUES (
    NEW.id,
    NEW.email,
  ...
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = dump_tracker, auth, pg_temp;

DROP TRIGGER IF EXISTS dt_on_auth_user_created ON auth.users;
CREATE TRIGGER dt_on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW
  EXECUTE FUNCTION dump_tracker.dt_handle_new_user();

-- Ensure dt_users row exists (call from client if trigger missed)
CREATE OR REPLACE FUNCTION dump_tracker.dt_ensure_user_exists()
RETURNS void AS $$
DECLARE
...
BEGIN
  ...
  SET
    email = COALESCE(EXCLUDED.email, dump_tracker.dt_users.email),
    first_name = CASE 
      WHEN dump_tracker.dt_users.first_name IS NULL OR dump_tracker.dt_users.first_name = '' 
      THEN COALESCE(EXCLUDED.first_name, '')
      ELSE dump_tracker.dt_users.first_name
    END,
    last_name = CASE 
      WHEN dump_tracker.dt_users.last_name IS NULL OR dump_tracker.dt_users.last_name = '' 
      THEN COALESCE(EXCLUDED.last_name, '')
      ELSE dump_tracker.dt_users.last_name
    END;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = dump_tracker, auth, pg_temp;

GRANT EXECUTE ON FUNCTION dump_tracker.dt_ensure_user_exists() TO authenticated;

-- Enable Row Level Security
ALTER TABLE dump_tracker.dt_users ENABLE ROW LEVEL SECURITY;
ALTER TABLE dump_tracker.dt_locations ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "dt_users_select_own" ON dump_tracker.dt_users;
CREATE POLICY "dt_users_select_own"
  ON dump_tracker.dt_users FOR SELECT
  USING (auth.uid() = id);

DROP POLICY IF EXISTS "dt_users_insert_own" ON dump_tracker.dt_users;

DROP POLICY IF EXISTS "dt_users_update_own" ON dump_tracker.dt_users;
CREATE POLICY "dt_users_update_own"
  ON dump_tracker.dt_users FOR UPDATE
  USING (auth.uid() = id)
  WITH CHECK (auth.uid() = id);

DROP POLICY IF EXISTS "dt_locations_select_own" ON dump_tracker.dt_locations;
CREATE POLICY "dt_locations_select_own"
  ON dump_tracker.dt_locations FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "dt_locations_insert_own" ON dump_tracker.dt_locations;
CREATE POLICY "dt_locations_insert_own"
  ON dump_tracker.dt_locations FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "dt_locations_update_own" ON dump_tracker.dt_locations;
CREATE POLICY "dt_locations_update_own"
  ON dump_tracker.dt_locations FOR UPDATE
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "dt_locations_delete_own" ON dump_tracker.dt_locations;
CREATE POLICY "dt_locations_delete_own"
  ON dump_tracker.dt_locations FOR DELETE
  USING (auth.uid() = user_id);

ALTER TABLE dump_tracker.dt_entries ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "dt_entries_select_own" ON dump_tracker.dt_entries;
CREATE POLICY "dt_entries_select_own"
  ON dump_tracker.dt_entries FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "dt_entries_insert_own" ON dump_tracker.dt_entries;
CREATE POLICY "dt_entries_insert_own"
  ON dump_tracker.dt_entries FOR INSERT
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "dt_entries_update_own" ON dump_tracker.dt_entries;
CREATE POLICY "dt_entries_update_own"
  ON dump_tracker.dt_entries FOR UPDATE
  USING (auth.uid() = user_id)
  WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "dt_entries_delete_own" ON dump_tracker.dt_entries;
CREATE POLICY "dt_entries_delete_own"
  ON dump_tracker.dt_entries FOR DELETE
  USING (auth.uid() = user_id);

ALTER TABLE dump_tracker.dt_notifications ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "dt_notifications_select_opt_in" ON dump_tracker.dt_notifications;
CREATE POLICY "dt_notifications_select_opt_in"
  ON dump_tracker.dt_notifications FOR SELECT
  USING (
    (SELECT leaderboard_opt_in FROM dump_tracker.dt_users WHERE id = auth.uid()) = TRUE
  );

-- Data API (PostgREST): explicit grants required for new Supabase projects (from 2026-05-30)
-- and for new tables on existing projects (from 2026-10-30). RLS still governs row access.
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_sync_location_count()
RETURNS TRIGGER AS $$
BEGIN
  UPDATE dump_tracker.dt_locations
  SET count = (
    SELECT COUNT(*)
    FROM dump_tracker.dt_entries
    WHERE dump_tracker.dt_entries.location_id = COALESCE(NEW.location_id, OLD.location_id)
  ),
  updated_at = NOW()
  WHERE id = COALESCE(NEW.location_id, OLD.location_id);
  RETURN COALESCE(NEW, OLD);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = dump_tracker, pg_temp;

DROP TRIGGER IF EXISTS dt_sync_count_on_insert ON dump_tracker.dt_entries;
CREATE TRIGGER dt_sync_count_on_insert
  AFTER INSERT ON dump_tracker.dt_entries
  FOR EACH ROW
  EXECUTE FUNCTION dump_tracker.dt_sync_location_count();

DROP TRIGGER IF EXISTS dt_sync_count_on_delete ON dump_tracker.dt_entries;
CREATE TRIGGER dt_sync_count_on_delete
  AFTER DELETE ON dump_tracker.dt_entries
  FOR EACH ROW
  EXECUTE FUNCTION dump_tracker.dt_sync_location_count();
...
-- ============================================================================
CREATE OR REPLACE FUNCTION dump_tracker.dt_notify_on_entry_insert()
RETURNS TRIGGER AS $$
DECLARE
  u_opt_in BOOLEAN;
  u_first TEXT;
...
BEGIN
  ...
  SELECT COUNT(*) INTO entry_count FROM dump_tracker.dt_entries WHERE location_id = NEW.location_id;
  SELECT location_name INTO loc_name FROM dump_tracker.dt_locations WHERE id = NEW.location_id;
  IF entry_count = 1 AND loc_name IS NOT NULL THEN
    INSERT INTO dump_tracker.dt_notifications (type, actor_user_id, payload)
    SELECT 'first_dump_at_location', NEW.user_id, jsonb_build_object('location_name', loc_name)
    WHERE NOT EXISTS (
      SELECT 1 FROM dump_tracker.dt_notifications n
  ...
  IF loc_name IS NOT NULL THEN
    ...
    FROM dump_tracker.dt_entries de
    INNER JOIN dump_tracker.dt_locations d ON d.id = de.location_id
  ...
  FROM (
    ...
    FROM dump_tracker.dt_entries de
    INNER JOIN dump_tracker.dt_users u ON u.id = de.user_id AND u.leaderboard_opt_in = TRUE
    WHERE de.id != NEW.id
...
DROP TRIGGER IF EXISTS dt_z_notify_on_entry_insert ON dump_tracker.dt_entries;
CREATE TRIGGER dt_z_notify_on_entry_insert
  AFTER INSERT ON dump_tracker.dt_entries
  FOR EACH ROW
  EXECUTE FUNCTION dump_tracker.dt_notify_on_entry_insert();

-- Wipes milestone on UPDATE when wipes column changes (fire when crossing 500 boundary)
CREATE OR REPLACE FUNCTION dump_tracker.dt_check_wipes_milestone_on_update()
RETURNS TRIGGER AS $$
DECLARE
  total_wipe_count BIGINT;
  total_wipe_prev BIGINT;
...
DROP TRIGGER IF EXISTS dt_notify_on_entry_update_wipes ON dump_tracker.dt_entries;
CREATE TRIGGER dt_notify_on_entry_update_wipes
  AFTER UPDATE ON dump_tracker.dt_entries
  FOR EACH ROW
  WHEN (OLD.wipes IS DISTINCT FROM NEW.wipes)
  EXECUTE FUNCTION dump_tracker.dt_check_wipes_milestone_on_update();
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_daily()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
BEGIN
  ...
  FROM dump_tracker.dt_users u
  INNER JOIN dump_tracker.dt_entries de ON de.user_id = u.id
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_weekly()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_2026()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_ghost_wipes()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_messy_dumps()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_liquid_dumps()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_explosive_dumps()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_max_wipes()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
BEGIN
  ...
  WITH user_max AS (
    ...
    FROM dump_tracker.dt_entries de
    INNER JOIN dump_tracker.dt_users u ON u.id = de.user_id
    WHERE u.leaderboard_opt_in = TRUE
      AND u.first_name IS NOT NULL
      AND u.last_name IS NOT NULL
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_total_wipes()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_weekly_wipes_top3()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_single_day_record()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_single_location_record()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
BEGIN
  ...
  SELECT 
    ...
    COUNT(de.id)::BIGINT as dump_count
  FROM dump_tracker.dt_users u
  INNER JOIN dump_tracker.dt_locations d ON d.user_id = u.id
  INNER JOIN dump_tracker.dt_entries de ON de.location_id = d.id
  WHERE u.leaderboard_opt_in = TRUE
    AND u.first_name IS NOT NULL
    AND u.last_name IS NOT NULL
    AND u.first_name != ''
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_avg_per_day()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_leaderboard_distinct_locations()
RETURNS TABLE (
  user_id UUID,
  first_name TEXT,
  last_name TEXT,
...
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_daily() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_weekly() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_2026() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_ghost_wipes() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_messy_dumps() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_liquid_dumps() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_explosive_dumps() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_max_wipes() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_total_wipes() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_weekly_wipes_top3() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_single_day_record() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_single_location_record() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_avg_per_day() TO authenticated;
GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_leaderboard_distinct_locations() TO authenticated;

-- Notifications feed RPC (opt-in users only)
CREATE OR REPLACE FUNCTION dump_tracker.dt_get_notifications(p_limit INT DEFAULT 50)
RETURNS TABLE (
  id UUID,
  type TEXT,
  payload JSONB,
...
BEGIN
  IF NOT (SELECT COALESCE(leaderboard_opt_in, FALSE) FROM dump_tracker.dt_users WHERE dump_tracker.dt_users.id = auth.uid()) THEN
    RETURN;
  END IF;
  ...
  SELECT
    ...
    u.first_name,
    u.last_name
  FROM dump_tracker.dt_notifications n
  INNER JOIN dump_tracker.dt_users u ON u.id = n.actor_user_id
  ORDER BY n.created_at DESC
  LIMIT p_limit;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = dump_tracker, auth, pg_temp;

GRANT EXECUTE ON FUNCTION dump_tracker.dt_get_notifications(INT) TO authenticated;
